#!/usr/bin/env python3
"""
BLP Homework - Complete Python Implementation
Economics 600a, Fall 2025
Marek Chadim (marek.chadim@yale.edu)

This file contains all executable Python code from the BLP homework.
Extracted from blp.ipynb notebook file.

The code implements:
- Data generation with simulated demand and cost shocks
- Market equilibrium price solving
- OLS, 2SLS, and Nested Logit estimation
- BLP random coefficients logit estimation with pyBLP
- Merger simulation and welfare analysis
- Publication-ready figures for welfare and merger comparisons

To run: python3 BLP_homework_code.py
"""
...
# Plot bars with professional colors (colorblind-friendly)
colors_merger = {
    'within': '#0173B2',  # Blue for within-nest
    'cross': '#ECE133',   # Yellow for cross-nest
}

bars1 = ax.bar(indices - width/2, pct_price_changes, width, 
               label='Within-Nest (Firms 1 & 2)', 
               color=colors_merger['within'], edgecolor='black', linewidth=0.5)
bars2 = ax.bar(indices + width/2, mean_pct_change_cross, width, 
               label='Cross-Nest (Firms 1 & 3)', 
               color=colors_merger['cross'], edgecolor='black', linewidth=0.5)
...
for idx, value in enumerate(pct_price_changes):
    ...
    ax.text(indices[idx] - width/2, value + y_offset, f'{value:.2f}%', 
...
# Add note about merger types
fig.text(0.5, -0.02, 
         'Note: Within-nest merger (1 & 2) involves two satellite providers. '
         'Cross-nest merger (1 & 3) involves one satellite and one wired provider.',
...
scenarios = ['Merger 1&2\n(No Efficiency)', 'Merger 1&2\n(15% Cost Cut)']
delta_cs = [total_delta_cs_no_eff, total_delta_cs_with_eff]
delta_ps = [delta_ps_no_eff, delta_ps_with_eff]
...
# Plot stacked bars with professional colors
# Using colorblind-friendly palette
colors = {
    'CS': '#0173B2',  # Blue for consumer surplus
    'PS': '#DE8F05',  # Orange for producer surplus
}

# Create bars
bars_cs = ax.bar(x_pos, delta_cs, width, label='Consumer Surplus (ΔCS)', 
                  color=colors['CS'], edgecolor='black', linewidth=0.5)
bars_ps = ax.bar(x_pos, delta_ps, width, bottom=delta_cs, 
                  label='Producer Surplus (ΔPS)', color=colors['PS'], 
                  edgecolor='black', linewidth=0.5)

# Add horizontal line at zero
ax.axhline(y=0, color='black', linestyle='-', linewidth=1.2, zorder=1)

# Add total welfare values on top of bars
for i, (cs, ps, w) in enumerate(zip(delta_cs, delta_ps, delta_w)):
    # Position label
    y_pos = w
    va = 'bottom' if w >= 0 else 'top'
    y_offset = 1500 if w >= 0 else -1500

    # Add total welfare label
    ax.text(i, y_pos + y_offset, f'ΔW = ${w:,.0f}', 
            ha='center', va=va, fontweight='bold', fontsize=10)

    # Add component values inside bars (if bars are large enough)
    if abs(cs) > 5000:
        ax.text(i, cs/2, f'${cs:,.0f}', ha='center', va='center', 
                color='white', fontweight='bold', fontsize=9)
    if abs(ps) > 5000:
        ax.text(i, cs + ps/2, f'${ps:,.0f}', ha='center', va='center', 
                color='white', fontweight='bold', fontsize=9)

# Formatting
ax.set_ylabel('Total Surplus Change ($)', fontweight='bold')
ax.set_title('Welfare Effects of Satellite Merger (Firms 1 & 2)', 
             fontweight='bold', pad=15)
ax.set_xticks(x_pos)
...
# Add note about market size
fig.text(0.5, -0.02, f'Note: Assumes {M_t:,} consumers per market across {T} markets. '
         f'Values represent aggregate changes across all markets.',
         ha='center', fontsize=9, style='italic', wrap=True)
...
# Save high-resolution figure
output_filename = 'surplus_changes_merger.pdf'
plt.savefig(output_filename, format='pdf', bbox_inches='tight', dpi=300)
print(f'✓ Saved publication-ready figure: {output_filename}')

# Also save as PNG for preview
plt.savefig('surplus_changes_merger.png', format='png', bbox_inches='tight', dpi=300)
print(f'✓ Saved PNG preview: surplus_changes_merger.png')
